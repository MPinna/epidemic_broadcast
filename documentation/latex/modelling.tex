...
success probability $p$. The probability of the active node not transmitting for
$k$ consecutive slots is a geometric distribution:
\begin{equation}
	P(X = k) = (1 - p)^{k}.
...
mean value of the geometric distribution, $\frac{1 - p}{p}$, since the central
...
remaining ones.
\subsection{Star configuration with all but one active node}
\label{ssec:star2}
\begin{figure}[H]
    \begin{center}
